 ...
 * Retrieved from https://developer.mozilla.org/en-US/docs/Web/HTTP/Status
 */
export const HttpStatus = {
  ...
  UNPROCESSABLE_ENTITY: { code: 422, message: "Unprocessable Entity" }, // Often used for validation errors
...
export function getHttpMessageForCode(code: number): string {
  const message = Object.values(HttpStatus).find((status) => status.code === code)?.message;
  if (!message) {
    throw new TypeError(`Unknown HTTP status code: ${code}`);
  }
  return message;
}
